...
class Cluster(ComponentResource):
    ...
    def __init__(self, context: Context, config: config.Config, *, opts: ResourceOptions | None) -> None:
    ...
    def build_kubernetes(self) -> None:
        self._kubernetes = kubernetes.Kubernetes(
            ...
            opts=self._child_opts.merge(
                ResourceOptions(
                    providers={"kubernetes": pulumi_kubernetes.Provider(self._name, kubeconfig=self._host.kubeconfig)}
                )
            ),
        )
